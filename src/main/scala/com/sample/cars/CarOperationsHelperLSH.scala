...
class CarOperationsHelperLSH(_ds: DataFrame,
  ...
  StopWordsRemover.loadDefaultStopWords(CarOperationsHelperLSH.DefaultLanguage)

  /**
    * Preparing df for further operations
    * 1. Replace nulls
    * 2. Concat principalComponentsColumns in a single column (useful for this sample -test)
    * 3. Replacing tabs, dost and commas
    *
    * @param df
    * @return
    */
  //TODO: Use principal components columns instead of hardcoded
  def preparedDataSet()(df: DataFrame): DataFrame = {
    df.na.fill("0").withColumn(
      CarOperationsHelperLSH.ConcatComments,
    ...
    ).withColumn(CarOperationsHelperLSH.ConcatComments,
      regexp_replace(col(CarOperationsHelperLSH.ConcatComments),
        "\\.|\\,|\\t", ""
      )
    )
  }
  ...
  def getWordsFilteredDF(df: DataFrame): (DataFrame, CountVectorizerModel) = {
    ...
    val pccTokenizer = new Tokenizer()
      .setInputCol(CarOperationsHelperLSH.ConcatComments)
      .setOutputCol(CarOperationsHelperLSH.ColumnWordsArray)
    val wordsArrayDF = pccTokenizer.transform(df)

    val remover = new StopWordsRemover()
      .setCaseSensitive(false)
      .setStopWords(CarOperationsHelperLSH.stopWords)
      .setInputCol(CarOperationsHelperLSH.ColumnWordsArray)
      .setOutputCol(CarOperationsHelperLSH.ColumnFilteredWordsArray)
    ...
    val vectorModeler: CountVectorizerModel = new CountVectorizer()
      .setInputCol(CarOperationsHelperLSH.ColumnFilteredWordsArray)
      .setOutputCol(CarOperationsHelperLSH.ColumnFeaturesArray)
      .setVocabSize(VocabularySHLSize)
      .setMinDF(10)
      .fit(wordsFiltered)

    val vectorizedCarsDF = vectorModeler.transform(wordsFiltered)
      .filter(validateEmptyVector(col(CarOperationsHelperLSH.ColumnFeaturesArray)))
      .select(col(CarOperationsHelperWindowStrategy.ConcatComments),
        col(CarOperationsHelperLSH.ColumnUniqueId),
        col(CarOperationsHelperLSH.ColumnFilteredWordsArray),
        col(CarOperationsHelperLSH.ColumnFeaturesArray))

    (vectorizedCarsDF, vectorModeler)
  }
  ...
  def deduplicateDataSet(df: DataFrame): (DataFrame, MinHashLSHModel) = {
    ...
    val minLshConfig = new MinHashLSH().setNumHashTables(hashesNumber)
      .setInputCol(CarOperationsHelperLSH.ColumnFeaturesArray)
      .setOutputCol(CarOperationsHelperLSH.hashValuesColumn)
